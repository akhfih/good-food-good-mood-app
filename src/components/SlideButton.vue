<template>
  <component :is="type" class="button">
    <div v-if="next === true" class="btn-next">
      NEXT
      <svg
        width="33"
        height="33"
...
.button {
  ...
  border: none;
  border-radius: 100px;
  background-color: map-get(map-get($color, primary), 1);
  outline: none;
  ...
  cursor: pointer;
  &:hover {
    background-color: map-get(map-get($color, avocado), 1);
